...
#include "../Inc/ParticleEngine2D.h"

ParticleEngine2D::ParticleEngine2D(RenderEngine2D* engine){
...
void ParticleEngine2D::makeParticleEffect()
{
    ...
    {
        this->temporaryParticleEffects[p]->life-=this->renderEngine->getDeltaTime();
        ...
        for(int i = 0; i<this->temporaryParticleEffects[p]->particles.size(); i++)
        {
            ...
            tempP.life -= this->renderEngine->getDeltaTime();
            // reduce life
            if (tempP.life > 0.0f)
            {
                // particle is alive, thus update
                tempP.pos += tempP.velocity * this->renderEngine->getDeltaTime();
                tempP.color.a -= (this->renderEngine->getDeltaTime()/this->temporaryParticleEffects[p]->partcLife);
            }
            if(tempP.life>0.0)
            {
                zeroParticles = 0;

                this->renderEngine->addParticle(tempP.pos+this->relativePos,
    ...
    for(int p = 0; p<this->constantParticleEffects.size(); p++)
    {
        ...
        for(int i = 0; i<this->constantParticleEffects[p]->particles.size(); i++)
        {
            ...
            if (tempP.life > 0.0f)
            {
                // particle is alive, thus update
                tempP.pos += tempP.velocity * this->renderEngine->getDeltaTime();
                tempP.color.a -= (this->renderEngine->getDeltaTime()/this->constantParticleEffects[p]->partcLife);
            }
            if(tempP.life>0.0)
            {
